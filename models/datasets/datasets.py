...
class PACS(object):
    ...
    def __init__(self):
        ...
        dataset_art = BaseDataset(image_art, lbl_art, domain_dic['APR'],data_transforms['test'])
        dataset_cartoon = BaseDataset(image_cartoon, lbl_cartoon, domain_dic['Photos'],data_transforms['test'])
        dataset_photo = BaseDataset(image_photo, lbl_photo, domain_dic['Multispectral'],data_transforms['test'])
        ...
        train_images_art = pd.read_csv(os.path.join(data_path, 'APR_train.txt'), header=None, sep=' ')
        image_train_art = data_path + '/' + train_images_art[0].values
        lbl_train_art = train_images_art[1].values - 1
        val_images_art = pd.read_csv(os.path.join(data_path, 'APR_val.txt'), header=None, sep=' ')
        image_val_art = data_path + '/' + val_images_art[0].values
        lbl_val_art = val_images_art[1].values - 1

        train_images_cartoon = pd.read_csv(os.path.join(data_path, 'Photos_train.txt'), header=None, sep=' ')
        image_train_cartoon = data_path + '/' + train_images_cartoon[0].values
        lbl_train_cartoon = train_images_cartoon[1].values - 1
        val_images_cartoon = pd.read_csv(os.path.join(data_path, 'Photos_val.txt'), header=None, sep=' ')
        image_val_cartoon = data_path + '/' + val_images_cartoon[0].values
        lbl_val_cartoon = val_images_cartoon[1].values - 1

        train_images_photo = pd.read_csv(os.path.join(data_path, 'Multispectral_train.txt'), header=None, sep=' ')
        image_train_photo = data_path + '/' + train_images_photo[0].values
        lbl_train_photo = train_images_photo[1].values - 1
        val_images_photo = pd.read_csv(os.path.join(data_path, 'Multispectral_val.txt'), header=None, sep=' ')
        ...
        dataset_train_art = BaseDataset(image_train_art, lbl_train_art, domain_dic['APR'], data_transforms['train'])
        dataset_train_cartoon = BaseDataset(image_train_cartoon, lbl_train_cartoon, domain_dic['Photos'], data_transforms['train'])
        dataset_train_photo = BaseDataset(image_train_photo, lbl_train_photo, domain_dic['Multispectral'], data_transforms['train'])
        #dataset_train_sketch = BaseDataset(image_train_sketch, lbl_train_sketch, domain_dic['sketch'], data_transforms['test'])

        dataset_val_art = BaseDataset(image_val_art, lbl_val_art, domain_dic['APR'], data_transforms['test'])
        dataset_val_cartoon = BaseDataset(image_val_cartoon, lbl_val_cartoon, domain_dic['Photos'], data_transforms['test'])
        dataset_val_photo = BaseDataset(image_val_photo, lbl_val_photo, domain_dic['Multispectral'], data_transforms['test'])
